...
- `[commentary]` `llm_polish` (default `false`) zapíná grounded generování přes OpenAI-compatible HTTP. Director vybere bezpečný autorský anchor a `commentary-facts/2` rozdělí povinná a volitelná fakta; syrový recent komentář se modelu neposílá. `llm_base_url` má default `http://127.0.0.1:11434/v1`, `llm_model` nově `qwen3:4b-instruct-2507-q4_K_M`, `llm_timeout_s=12.0`, `llm_temperature=0.45`, `llm_max_tokens=360` a `llm_max_attempts=5` (clamp 1–8). Model může napsat jednu až dvě věty do plného limitu nodu. Fact-lock odmítá nepovolené vztahy, nová jména/čísla a P/S markery; po timeoutu nebo vyčerpání retry se přečte autorský anchor.
...
**Migration:** `llm_model` default changes from `qwen2.5:3b` to `qwen3:4b-instruct-2507-q4_K_M`; an existing explicit value stays unchanged. Grounded generation now uses the full node TTS budget for one or two sentences instead of the skeleton-relative cap. `llm_timeout_s` remains the wall-clock budget for all retries and `llm_max_attempts` remains clamped to 1–8. Any failed or rejected generation speaks the safe authored anchor.
