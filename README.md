...
Overlay závislosti (`bleak`, `psutil`, `nvidia-ml-py`) jdou s `pip install -e .`. GPU čísla bere NVML. CPU package na Windows: LibreHardwareMonitor 0.9.5+ **zrušil WMI** — overlay čte `http://127.0.0.1:8085/data.json` (Options → Remote Web Server → Run). Starší LHM pořád WMI `root\LibreHardwareMonitor`. Stock Windows **nemá** CPU package power. FPS/FT berou iRacing — mimo 3D zůstanou prázdné.
