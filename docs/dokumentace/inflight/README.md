...
| #284 NarrativeRuntime actor reducer (library) | events + commentary mailbox | `src/irswitch/events/narrative_runtime.py` — [§ lookup](#284-narrative-runtime-lookup) | `tests/test_narrative_runtime.py` (**104**) |
...
- **Tests:** `tests/test_narrative_runtime.py` (**104**: 85 matrix rows + 19 focused, including effect-task ownership, cancel-drop, overflow/coalesce diagnostics, reason codes). Matrix loader reads packaged `actor-transition-model.json`; `machine/` hashes were not rewritten.
