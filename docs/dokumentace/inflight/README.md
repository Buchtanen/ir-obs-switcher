...
| #284 NarrativeRuntime actor reducer (library) | events + commentary mailbox | `src/irswitch/events/narrative_runtime.py` — [§ lookup](#284-narrative-runtime-lookup) | `tests/test_narrative_runtime.py` (**101**) |
...
- **Owned effect intents + library executor:** reducer still emits string intents (`effect:dispatch_realization`, `effect:dispatch_tts`, `effect:cancel_realization`, `effect:cancel_tts`, deadline cancel/arm). `NarrativeRuntime.apply_effects` / `run()` own at most one realization task and one TTS task; injectable `realization_effect` / `tts_effect` workers may only `admit` commands (no direct actor-state mutate). Cancel drops stale completions. Deadline arm/cancel strings remain intents (timer tasks still deferred).
...
- **Tests:** `tests/test_narrative_runtime.py` (**101**: 85 matrix rows + 16 focused, including effect-task ownership, cancel-drop, overflow/coalesce diagnostics, reason codes). Matrix loader reads packaged `actor-transition-model.json`; `machine/` hashes were not rewritten.
...
- **Still out of scope (issue OPEN):** live EventSubscription replacement, fanout/producer wiring, deadline timer tasks, live health/API schema cutover, master cutover, V4 overlay tape, composing downstream library slices into one live loop.
