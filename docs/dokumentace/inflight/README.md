...
| #349 live-path composition safety (slices 1–4) | race + events | Slices 1–3 on tip; Slice 4 Qwen/verifier fail-closed — [§ lookup](#349-live-path-qwen-failclosed-slice-4-lookup) · [`tests/test_live_path_qwen_failclosed.py`](../../../tests/test_live_path_qwen_failclosed.py) | **open** on `cursor/live-path-qwen-failclosed-349-cad3` (Slice 5 + §24.9 still pending; parent #234 / related #278) |
...
- **Issue:** [#349](https://github.com/Buchtanen/ir-obs-switcher/issues/349) — Slice 4 Qwen/verifier fail-closed: no silent template live fallback when `allow_qwen` is on; missing #270 verify frame rejects TTS when `SemanticVerifier` is attached (**open** PR; Slice 5 + §24.9 still pending).
- **Related:** [#278](https://github.com/Buchtanen/ir-obs-switcher/issues/278) offline **44/44** complete; live Windows gates blocked on #349 composition safety. Parent [#234](https://github.com/Buchtanen/ir-obs-switcher/issues/234).
- **Code:** `events/narrative_realization_bridge.py` (`build_realization_effect(..., allow_qwen=)` fail-closes on authored+Qwen miss; `SpeechDraftCache.take_authored`), `events/narrative_runtime.py` (missing frame → `realization_verify_rejected`), `race/runtime.py` comment.
- **Tests:** [`tests/test_live_path_qwen_failclosed.py`](../../../tests/test_live_path_qwen_failclosed.py); `tests/test_narrative_runtime.py::test_semantic_verifier_rejects_when_frame_missing`.
