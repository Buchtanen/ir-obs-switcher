...
`tests/test_narrative_ingress.py` (**45** = prior **44** + **1** health reason-code row feat `cffaab1`). `tests/test_narrative_runtime.py` (**146** = prior **144** + **2** live verify-frame rows feat `a1ba998`). `tests/test_coalesce_policy.py` (**3** feat `dd64047`). `tests/test_actor_transition_goldens.py` (**32** = **13** `raceTraces` + **10** `overflowScenarios` + **2** `orderingScenarios` + sync/assert rows + **2** runtime race rows + **3** overflow library evidence rows test `f465bfd`). Runtime+goldens **181**. Core subset **226** (= **146** + **45** + **32** + **3**). `tests/test_narrative_runtime_http.py` (**16** incl. disabled-subset loop asserts + **2** public-path cutover rows). `tests/test_commentary_http.py` (**7**). Related **318** (= prior **315** + **3** test `f465bfd`). Ingress+http **43** (= prior **42** + **1** health reason row).
