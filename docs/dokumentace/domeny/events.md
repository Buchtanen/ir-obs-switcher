...
> **Větev `cursor/narrative-runtime-284-matrix-cad3`:** tenký delta k master `events.md`. Full v2 narrative moduly: [inflight](../inflight/README.md). Identity: [§ golden-health identity](../inflight/README.md#284-273-golden-health-identity-slice-lookup). Speech/language/components: [§ golden-health speech](../inflight/README.md#284-273-golden-health-speech-slice-lookup).
...
`tests/test_narrative_ingress.py` (**11** = prior **8** + **3** speech projection rows). Related **201**.

## Still deferred (#284 OPEN)

Full #273 schema: decisions/validate/speak goldens + handlers, full components (detectors/facts), catalog/config/episodes/byTapeChannel; integrated loop liveness; master cutover.
