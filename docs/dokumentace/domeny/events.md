...
`tests/test_narrative_ingress.py` (**45** = prior **44** + **1** health reason-code row feat `cffaab1`). `tests/test_narrative_runtime.py` (**155** = prior **149** + **6** ordering/completion race rows test `5f4eb61`). `tests/test_narrative_tape_bridge.py` (**4** = prior **3** + **1** disable flush row test `d9089a1`). `tests/test_coalesce_policy.py` (**3** feat `dd64047`). `tests/test_actor_transition_goldens.py` (**32** = **13** `raceTraces` + **10** `overflowScenarios` + **2** `orderingScenarios` + sync/assert rows + **2** runtime race rows + **3** overflow library evidence rows test `f465bfd`). Runtime+goldens **187** (= **155** + **32**). Core subset **235** (= **155** + **45** + **32** + **3**). `tests/test_narrative_runtime_http.py` (**16** incl. disabled-subset loop asserts + **2** public-path cutover rows). `tests/test_commentary_http.py` (**7**). Related **331** (was **325**; test `5f4eb61`). Ingress+http **43** (= prior **42** + **1** health reason row).
...
**#284 stays OPEN** (**39/41** AC, **2** remain: upstream snapshot immutability + process TDD) — no master PR.
