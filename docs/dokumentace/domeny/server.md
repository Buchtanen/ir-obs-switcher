...
- Validate/speak: additive `POST /api/commentary/runtime/validate` + `POST /api/commentary/runtime/speak` — offline validate + `try_manual_speak`; legacy `POST /api/commentary/validate|speak` beze změny (viz `API.md` + [§ golden-health validate/speak](../inflight/README.md#284-273-golden-health-validatespeak-slice-lookup)).
...
`tests/test_api.py` — `/health` obsahuje `commentary`; `tests/test_narrative_runtime_http.py` — decisions + validate/speak mounts (**13** rows); related suite **215** s narrative ingress identity + speech + decision + validate projection rows.
