...
`tests/test_api.py` — `/health` obsahuje `commentary`; `tests/test_narrative_runtime_http.py` — decisions + validate/speak/cutover + admission-timeout + disabled-subset loop asserts (**16** rows); `tests/test_commentary_http.py` (**7** cutover rows); related suite **252** s narrative ingress identity + timeline session identity (null + live) + speech + decision + validate + status_ready (stub + live config/episodes/byTapeChannel) + llm/tts components + loop heartbeats + latch + public validate/speak cutover rows; ingress+http **42** (= prior **40** + **2** public-path cutover rows).
