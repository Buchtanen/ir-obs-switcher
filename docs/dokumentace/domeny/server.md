...
`tests/test_api.py` — `/health` obsahuje `commentary`; `tests/test_narrative_runtime_http.py` — decisions + validate/speak + admission-timeout mounts (**14** rows); related suite **219** s narrative ingress identity + speech + decision + validate projection + latch rows.
