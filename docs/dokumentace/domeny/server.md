...
- Validate/speak: additive `POST /api/commentary/runtime/validate` + `POST /api/commentary/runtime/speak` — offline validate + `try_manual_speak` s `ManualAdmissionLatch` (503 `admission_timeout` on latch timeout); public `POST /api/commentary/validate|speak` cut over na NarrativeRuntime handlery (feat `f2c1f1b`; `/runtime/*` alias). Manual speak body frozen to exact keys `schemaVersion`/`text`/`language` only; unknown fields → **400** `invalid_request`; normalized `text` 1–400 no controls → else **422**; **202** `admittedState` exactly `committed` (branch `cursor/manual-speak-body-273-cad3` — [§ manual speak body](../inflight/README.md#273-manual-speak-body-slice-lookup)) (viz `API.md` + [§ golden-health validate/speak](../inflight/README.md#284-273-golden-health-validatespeak-slice-lookup) · [§ ManualAdmissionLatch](../inflight/README.md#284-273-manual-admission-latch-slice-lookup)).
