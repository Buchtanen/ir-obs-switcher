...
9. Na v2 větvi: handover do `docs/v2.0.0/implementation-handover.md`; modul lookup pro #253 v [inflight § CLOSING detector](inflight/README.md#253-closing-temporal-detector-lookup) a #254 v [inflight § UNDER_PRESSURE detector](inflight/README.md#254-under_pressure-temporal-detector-lookup). **Žádný PR do `master`** do cutoveru. #253 CLOSING je closed (first SHA `5362c33`). #254 UNDER_PRESSURE je implementované (first SHA `13e01ef`), issue stále open. #255 nespouštět, dokud člověk neřekne. NarrativeRuntime neaktivovat.
