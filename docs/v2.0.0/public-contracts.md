...
The tuning policy matrix is exact:

| Detector/catalog state | `production` | `calibration` | Recorder/capture loss |
| --- | --- | --- | --- |
| `tuning.none` | enabled value follows catalog/config | same | no detector effect; ordinary tape health only |
| `tuning.optional` | enabled value follows catalog/config | same | detector remains enabled; missing evidence is explicitly incomplete |
| `experimental=false,tuning.required` | config invalid when enabled | config invalid when enabled | unreachable |
| `experimental=true,tuning.required`, disabled | legal | legal | remains disabled |
| `experimental=true,tuning.required`, enabled | config invalid | legal only when tape is enabled, `detector_tuning` is selected, detector is allowlisted, input-window capture is enabled and writable recorder preflight succeeds before the run | protected `TAPE_HEALTH_CHANGED(capture_unavailable)` bundle disables only affected detectors for the rest of the run |

`required` is a calibration safety policy, not a production operating mode. Recorder queue loss, write/flush/rotation failure, path loss or any missing required pre/post-window record all count as capture loss. Recovery never re-enables within the current run; successful preflight plus the next stream boundary is required. The exact cross-layer fanout and ordering are frozen in `actor-transition-contract.md`.
...
The machine projection is `machine/api-contract.json`; `machine/api-contracts.schema.json` closes the eight public payload definitions and imports the byte-equivalent shared AtomicFact definition, while `machine/api-goldens.json` pins successful, degraded, rejected, error, transport, limit-clamp and removed-route examples. The checker additionally enforces registry-backed beat, predicate, attribute, relation, reason and tape-channel references plus cross-field invariants that JSON Schema cannot express.
...
~~~json
{
  ...
  "config": {
    ...
    "desiredGeneration": 7,
    "desiredHash": "sha256:1111111111111111111111111111111111111111111111111111111111111111",
    "effectiveHash": "sha256:2222222222222222222222222222222222222222222222222222222222222222",
    "applySequence": 12,
    "pendingChanges": [
  ...
  "components": {
    ...
    "tape": {"status": "disabled", "reason": null, "path": null, "size": 0, "drops": 0, "dropsByPriority": {"sample": 0, "normal": 0, "critical": 0}, "purposeCounts": []},
...
Enums: status is `disabled|starting|ready|degraded|stopping|stopped`; streamState is `inactive|active|unknown`; `streamActive` is its lossless OBS projection `false|true|null`; `narrativeRunActive` is a required boolean. Speech state is `idle|building|committed|speaking|stopping`. Episode `retainedCurrentCapacity` applies to the sum of candidate+active+suspended entries. Before the first observed output, `broadcastEpoch=0`; before the first admitted narrative run, `streamEpoch=0`. After a run closes, `streamEpoch` retains the last allocated value while `narrativeRunActive=false`; the next run increments it. `sessionPlan` is null before first plan publication; otherwise it is the exact bounded status projection `{revision,valid,reason,stages}`. Revision is nonnegative, stages has 0–3 unique values in canonical order, a valid plan has 1–3 stages and null reason, and an invalid plan has no stages plus `session_plan_conflict`. Whenever there is no coherent supported current session, `sessionRef`, `occurrenceId`, `lineageId` and `stage` are `null` and `historyComplete=false`, but the independently published session plan remains visible. Otherwise stage is `practice|qualifying|race`; unsupported/identity-conflict detail belongs in `reason`, not a fabricated stage. `config.applySequence` is nonnegative and names the effective snapshot. `config.pendingChanges` has 0–128 exact `{key,boundary,desiredGeneration}` entries sorted by key and never exposes values; desired/effective hashes may differ until every named boundary occurs. `byTapeChannel` contains known channels whose six-counter object has any nonzero value and is capped at 128 entries sorted by channel ID; every included object still contains all exact keys `kick,accepted,queued,selected,started,expired`, including zeros. Counters are nonnegative, monotonic within the current `streamEpoch`, reset on the next run and follow the exact funnel identities/increment rules in `schema-contracts.md`; non-applicable stages are omitted from rate calculations rather than treated as failures.

**Implementation note (#273, feat `466c2f3`, PR [#291](https://github.com/Buchtanen/ir-obs-switcher/pull/291)):** `StatusResponse` exposes counters only — no funnel-rate JSON fields. Live HTTP projection uses `OpportunityQueue.tape_channel_status_counts()` → shared `project_by_tape_channel_status` (zero-row filter, sorted keys, cap 128). Cohort-valid ratios for tooling/tests live in pure helper `cohort_funnel_rates(counters)` (`events/opportunity_queue.py`): returns `{kickToAccepted, acceptedToQueued, selectedToStarted}` with `null` for N/A denominators (e.g. visual-only after `accepted`, silence/successor without kick cohort).

Component status is `disabled|starting|ready|degraded|unavailable`; component `reason` values and all terminal/decision reasons are IDs from the frozen reason registry, never exception messages. LLM/TTS worker/backend generations, their `configGeneration`, and fact view revisions/counts are nonnegative integers. Component `configGeneration` names the desired generation whose effective component values its current preflight proves; while a newer applied component generation is pending/failed, that component cannot be used. TTS quarantined generation is null or no greater than the current backend generation; while equal, TTS must be unavailable and admit no speech. Tape `size` is the nonnegative byte size of the current open NDJSON body (0 when no file is open). `drops` equals the sum of the three nonnegative `dropsByPriority` counters (cumulative out-of-queue loss for the current narrative run). `purposeCounts` is a sorted 0–3 array of `{purposeChannel,count}` rows for `flow|llm_eval|detector_tuning` with positive counts only. `detectors.disabled` is capped at 128 entries of `{id, reason}` sorted by detector ID. Fact health becomes degraded with `fact_capacity_evicted` after lossy compaction for the rest of the narrative run and unavailable with `fact_capacity_exhausted` while no complete bounded view can be published. The next complete coherent bounded view moves unavailable→degraded; only a new complete run with no loss restores ready. Model/voice/path strings are bounded to their config maxima; tape path is relative to the configured recording root and never exposes an absolute host path.
...
Decision is `selected|silence|discarded|replaced|expired|invalidated`; candidateSource is `event_opportunity|story_successor|episode_beat|filler`; candidateOrder uses the exact actor-assigned pair from the planning contract; relation is one of `opens_episode|updates_active_episode|resolves_active_episode|continues_focused_episode|conflicts_with_focused_episode|independent_of_focused_episode` or `null`. A `silence` record has all candidate-specific fields null; every other decision has non-null beat, episode, tape channel, candidate source/order, urgency and score, while opportunity, relation, runner-up and terminal reason remain nullable according to the decision. The endpoint does not return prompt/completion content; that belongs to explicitly enabled tape capture.
...
ValidateRequest top-level keys are exactly `schemaVersion`, `text`, `beatId`, `evaluationAtMonoMs`, `actorBindings`, `factBindings` (`commentary-runtime/2`). Branch implementation (#273) parses each `factBindings` row via `AtomicFact.from_dict` and the frozen fact registry; incomplete rows, unknown fields, and unregistered predicates raise `ContractViolation` → HTTP 400 `invalid_request`.

Each returned claim verdict is `supported|unsupported|ambiguous`. The response `valid` flag is true exactly when no issue has `severity=error`; warnings alone do not make the response invalid.
