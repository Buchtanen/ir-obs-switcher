...
**Status:** design-freeze candidate owned by issues #235–#239, #245 and #261 (**#235–#239, #245 and #261 closed** on branch)
...
Detector states are `inactive|candidate|active|clearing`. Predicate results are `true|false|unknown`; each value names its registered feature/predicate ID and typed value/unit. `wouldEmitEventKind` is nullable. `parameterSnapshotId` resolves exactly one manifest snapshot for this detector/config hash. `windowFrameRange`, when captured, is exactly `{firstFrameSequence,lastFrameSequence,postWindowComplete}` with positive inclusive ordered endpoints. Every retained frame in that range is a separate `feature_frame` tape record with matching stream/occurrence/lineage/correlation identity. Optional capture may have gaps reported by drop accounting; required capture requires the entire declared range and any loss invokes the composition-owned disable path frozen in the actor contract.

`coverage[0..16]` is the bounded bucket-coverage trace from the shared directional algorithm in `detector-catalog-freeze.md` and F28, not a second copy of `gap.trend.coverage`. Window-level coverage remains the registered `gap.trend.coverage` FeatureValue. Each coverage row is exactly `{bucketStartMonoMs,bucketDurationS,coveredDurationS,sampleCount,usable}`. `bucketStartMonoMs` is the monotonic start of one sample bucket. `bucketDurationS` is that observation's `bucket_s`. `coveredDurationS` is the summed sample contribution in the bucket: each sample adds only the elapsed interval until the next expected sample, capped at `sample_interval_s`, and a lone sample never fills the rest of the bucket. `0 <= coveredDurationS <= bucketDurationS`. `sampleCount` is the number of valid raw samples in the bucket. `usable` is true exactly when `coveredDurationS >= bucketDurationS × min_coverage` from the same parameter snapshot; only usable buckets supply median/midpoint inputs to slope and net-closing. Rows sort uniquely by `bucketStartMonoMs`. The array persists at most the newest 16 buckets of the current trend window; detector math may still use an older suffix of the same window, and that truncation is visible because the window FeatureValue can disagree with a sum of the taped rows. A bucket cannot be carried across a relation epoch, occurrence, pit/tow/teleport invalidation or process instance.
...
`redactionPolicy` is exactly `{sensitiveValues,prompt,completion}`. `sensitiveValues` is the constant `marker_only`: the frozen sensitive key set in the config projection/`config_applied` patch uses `{key,redacted:true}` and never a value, while identity hashes keep the real normalized values. `prompt` is `none|hash|full` and `completion` is boolean; they are the same pair as RealizationRequest `capturePolicy` from `qwen-transport-contract.md`, snapshotted after `next_request` into this file. `prompt=full` is legal only when `enabledPurposeChannels` contains `llm_eval`. The policy names how this file redacts; it does not duplicate the frozen sensitive-key list or the tape-channel/detector allowlists, which live in the effective config projection.
...
At capacity, an incoming sample is dropped; normal first evicts oldest sample else is dropped; critical first evicts oldest sample, then oldest normal. If all queued records are critical, the incoming critical record is represented in the loss accumulator and writer health becomes degraded. Loss of any required-detector record additionally emits the out-of-queue CaptureHealthNotice frozen in the actor contract; the composition coordinator, never TapeWriter or NarrativeRuntime, disables only the affected experimental detector and atomically publishes its health/context bundle. Tape loss never blocks or mutates world/narrative truth directly.

The bounded out-of-queue `TapeLossAccumulator` payload is exactly `{buckets[1..192],firstLostMonoMs,lastLostMonoMs,firstLostReducerSequence?,lastLostReducerSequence?,configTransitions[0..128]}`. A bucket is exactly `{recordType,recordPriority,reason,count}` where reason is `tape_queue_drop|tape_write_failed|tape_flush_timeout|config_transition_lost`; buckets sort uniquely by record type, priority and reason. Monotonic endpoints are ordered. Reducer endpoints are both null or both non-null and ordered; null means all represented losses were outside actor order. A config transition row is exactly `{applySequence,oldEffectiveHash,newEffectiveHash}`, sorted uniquely by apply sequence. The accumulator is flushed in one exact `drop-notice/2` payload `{noticeId,loss}` at the next possible write and is also copied into the trailer or operational error path.

The exact `manifest-trailer/2` payload is `{finalRecordHash?,fileHash,recordCounts[0..16],purposeCounts[0..3],tapeChannelCounts[0..36],dropCounts[0..4],lossAccumulator?,lastReducerSequence?,closeReason,complete}`. Count rows are respectively `{recordType,count}`, `{purposeChannel,count}`, `{tapeChannel,count}` and `{reason,count}`, sorted uniquely by their ID. Nullable fields are present with null when no prior record, loss or actor sequence exists. `closeReason` is `rotation_size|rotation_duration|broadcast_ended|commentary_disabled|recording_disabled|shutdown|write_failed|flush_timeout`. `finalRecordHash` is the canonical JSON hash of the final non-trailer record, or null for an empty body. `fileHash` covers the exact UTF-8 manifest and non-trailer NDJSON bytes including their line terminators; it excludes the trailer line, avoiding a circular identity, and becomes the next continuation manifest's `previousFileHash`. `complete=true` requires a clean close with no accumulated loss; a missing/invalid trailer, hash mismatch, false completeness flag or non-null loss makes the file incomplete but does not make prior independently valid NDJSON records unreadable.
...
| config/runtime health | `disabled_by_config`, `disabled_invalid_config`, `catalog_invalid`, `legacy_key`, `starting`, `ready`, `component_preflight_pending`, `component_preflight_failed`, `component_unavailable`, `admission_timeout`, `session_identity_conflict`, `session_plan_conflict`, `fact_capacity_evicted`, `fact_capacity_exhausted`, `obs_state_unknown`, `history_incomplete`, `duck_unavailable` |
