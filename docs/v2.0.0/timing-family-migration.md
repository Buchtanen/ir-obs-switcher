# #275 Timing family migration (lap / SF / sector)

**Status:** Slice 1 — lap/SF + sector source inventory (all `legacy`; no `FAMILY_ROUTE` flip).  
...
**Lookup:** [inflight § #275](../dokumentace/inflight/README.md#275-timing-family-map-slice-1-lookup) · [events branch delta](../dokumentace/domeny/events.md#timing-family-migration-map-contractstiming_family_mappy)
...
Helpers: `timing_family_rows()`, `row_for_wire_id()`, `rows_by_migration_status()`, `migration_status_by_wire_id()`, `lap_complete_is_not_race_finish()`.
...
Deferred inventory (examples): personal best / gain / time lost, projected lap, invalid lap, qualifying attempt/projection, practice→quali→race recaps, EN patterns / TTS slots, restart/rewind replay cases.
