...
- Packaged copies of `beat-catalog.json`, `successor-graph.json` and `realization-pattern-cards.json` plus `coverage-matrix-replay-refs.json` are audited at runtime by `irswitch.contracts.coverage_matrix` (`tests/test_coverage_matrix.py`). That auditor does not rewrite these hashes.
...
- `vertical-slice-fixtures.json`, `vertical-slice-mutations.json` and `build_vertical_slice_fixtures.py` materialize all F01–F44 inputs/assertions/tape order with seven owning-contract hashes and executable boundary math.
...
python3 docs/v2.0.0/machine/build_vertical_slice_fixtures.py
```
...
Vertical slice fixtures OK: 44 scenarios, 14 executable calculations, 7 contract hashes each, 15 rejected mutations
```
