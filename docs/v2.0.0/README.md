...
**Status:** Wave C #247–#256 and Wave D #257–#262 plus #263 and #283 closed on this branch; #264 closed; #265 closed; #266 closed; #267 closed; #268 closed; #269 closed; #271 closed. [#284](https://github.com/Buchtanen/ir-obs-switcher/issues/284) **OPEN** — library StoryDirector composition + shadow NarrativeIngress + default-off NarrativeShadowConsumer slice landed on branch `cursor/narrative-runtime-284-matrix-cad3` (`events/narrative_runtime.py` **124** + `events/narrative_ingress.py` **5** + `events/narrative_shadow_consumer.py` **6** = **135** related tests: matrix + effect executor + silence/validity/realization/speech deadline timers + optional FreshnessGate/OpportunityQueue/EpisodeRegistry/StoryDirector composition + shadow fanout→mailbox ingress + `project_runtime_status` subset + default-off race shadow consumer; NarrativeShadowConsumer feat SHA `eaa58c9`; prior NarrativeIngress feat `13b744a`; prior StoryDirector feat `5fb8041`; prior EpisodeRegistry slice `5d59554`; prior OpportunityQueue slice `b306deb`; prior FreshnessGate slice `08472ed`); live EventSubscription cutover / health HTTP mount deferred. Not shipped on `master`.
