...
**Status:** Wave C #247–#256 and Wave D #257–#262 plus #263 and #283 closed on this branch; #264 closed; #265 closed; #266 closed; #267 closed; #268 closed; #269 closed; #271 closed. [#284](https://github.com/Buchtanen/ir-obs-switcher/issues/284) **OPEN** — library StoryDirector composition + shadow NarrativeIngress + default-off NarrativeShadowConsumer + commentary-runtime HTTP status mount + shadow fanout cutover reduce+status attach slice landed on branch `cursor/narrative-runtime-284-matrix-cad3` (`events/narrative_runtime.py` **124** + `events/narrative_ingress.py` **5** + `events/narrative_shadow_consumer.py` **6** + `events/narrative_runtime_http.py` **5** + `tests/test_narrative_fanout_cutover.py` **5** = **145** related tests: matrix + effect executor + silence/validity/realization/speech deadline timers + optional FreshnessGate/OpportunityQueue/EpisodeRegistry/StoryDirector composition + shadow fanout→mailbox ingress + fanout→`reduce_next()` cutover + process-level HTTP attach + `project_runtime_status` subset + `GET /api/commentary/runtime` (disabled library snapshot by default; process attach when shadow enabled) + default-off race shadow consumer; shadow fanout cutover feat SHA `975d3e1`; prior NarrativeRuntime HTTP feat `f4d1725`; prior NarrativeShadowConsumer feat `eaa58c9`; prior NarrativeIngress feat `13b744a`; prior StoryDirector feat `5fb8041`; prior EpisodeRegistry slice `5d59554`; prior OpportunityQueue slice `b306deb`; prior FreshnessGate slice `08472ed`); live CommentaryConsumer EventSubscription cutover deferred. Not shipped on `master`.
...
- [ ] [#284 — v2: single-owner NarrativeRuntime actor and command lifecycle](https://github.com/Buchtanen/ir-obs-switcher/issues/284) — owns mailbox ordering, effects, recovery and shutdown; depends on #237, #238, #240, #243, #245, #258, #262, #264, #265 and #269. **Branch library slice (OPEN):** reducer matrix + owned realization/TTS effect tasks + silence/validity/realization/speech deadline timers + optional FreshnessGate/OpportunityQueue/EpisodeRegistry/StoryDirector composition + shadow `NarrativeIngress` fanout→mailbox adapter + fanout→`reduce_next()` cutover + process-level HTTP attach + `project_runtime_status` subset + `GET /api/commentary/runtime` HTTP mount (disabled library snapshot by default) + default-off `NarrativeShadowConsumer` in `race.runtime` — [inflight § #284 runtime](../dokumentace/inflight/README.md#284-narrative-runtime-lookup) · [inflight § #284 ingress shadow](../dokumentace/inflight/README.md#284-narrative-ingress-shadow-lookup) · [inflight § #284 shadow consumer](../dokumentace/inflight/README.md#284-narrative-shadow-consumer-lookup) · [inflight § #284 HTTP mount](../dokumentace/inflight/README.md#284-narrative-runtime-http-lookup) · [inflight § #284 fanout cutover](../dokumentace/inflight/README.md#284-shadow-fanout-cutover-lookup); live CommentaryConsumer EventSubscription cutover not done.
