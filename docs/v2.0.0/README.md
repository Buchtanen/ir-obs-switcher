...
- [ ] [#274 — v2: migrate race outcome event families](https://github.com/Buchtanen/ir-obs-switcher/issues/274) — position/pass/overtake/leader/finish. **Slice 5 in progress:** shadow activation for `position`/`session` via `FAMILY_ROUTE` + race-outcome routing fix; see [race-outcome-migration.md](race-outcome-migration.md).
