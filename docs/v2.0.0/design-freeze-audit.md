...
**NOT READY FOR RUNTIME EDITS.** The architecture is feasible, but the unchecked artifacts below are real blockers, not implementation details. The exact 64-beat catalog projection, core DTO JSON Schemas and structural goldens are materialized. Successor graph, config/API, detector, actor-model, catalog-loader, controlled-English and end-to-end executable fixtures remain open. Issue #235 must remain open until every blocking artifact is reviewed, committed and pushed.
...
| Prompt compilation | “Compiled prompt” had no exact byte/content boundary and could absorb unrelated examples or mutable state | Versioned literal base/output blocks, one family/pattern selection and one canonical immutable data projection with hard UTF-8 bounds |
| Qwen wire subset | OpenAI-compatible transport did not define streaming, redirects/proxies, choice count or finish semantics | Exact SSE request body and fail-closed parser; one choice, visible content only, `finish_reason=stop`, no redirects/proxies or parsing fallback |
| Qwen liveness | Relying only on an HTTP callback could strand the sole building lane | Actor-owned protected monotonic deadline races results through the same mailbox and cancellation atomically detaches the sole logical worker token |
| Qwen latency | TTFT/total were named but clock milestones, nullability and warm/cold grouping were ambiguous | Exact same-process equations for admission/TTFB/TTFT/generation/total/reducer lag; missing observations remain null and residency groups stay separate |
...
| TTS request identity | Backend adapters had prose-only inputs, so voice/device/config or source identity could change after dispatch | Exact immutable `tts-utterance/2` snapshots text hash, narrative/manual identity, concrete backend generation, effective config and playback/duck limits |
| TTS callback order | Completion-before-acceptance, duplicate acceptance and late terminal behavior were not closed across backends | Exact `tts-callback/2` token/sequence protocol, one actor terminal, protocol-violation cancellation and stale/quarantined no-ops |
| Duck ownership | It was unclear whether actor, OBS scene logic or backend owned attenuation and restoration | TTS worker token owns best-effort duck-before-accept and one idempotent best-effort restore request; outcome is bounded, recorded and never overclaimed |
