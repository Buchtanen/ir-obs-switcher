...
The pre-implementation prompt, complete authored/Qwen `realization-request/2` objects, SSE boundary cases, warm-up and deadline-race goldens are materialized in `machine/qwen-transport-goldens.json` and checked with the controlled-English bundle. The SSE boundary evidence crosses the 2,048-byte visible-output, 16,384-byte frame and 65,536-byte stream limits independently and checks their exact rejection reason; split UTF-8 is fed through an incremental decoder. The repeatable target probe is `machine/run_qwen_latency.py`; it labels residency explicitly, disables proxies/redirects, performs no retries and emits raw plus median/P90/P95/max timing evidence.
