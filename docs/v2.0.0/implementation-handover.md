...
- Editing owner: current root agent through the #238 persistent ConfigLedger reload-owner checkpoint.
- Dirty scope: ledger coordinator/startup/API wiring, generation/preflight projection, API docs, tests and this handover.
...
1. Commit/push the persistent ConfigLedger reload-owner checkpoint and require green CI.
2. Append that SHA/CI to the existing dated #238 dev diary; do not create a second dated comment.
3. Remove the remaining legacy commentary keys and graph-runtime selector from live-reload/API schema controls; retain diagnostics only through the frozen migration table.
4. Audit the issue checklist against executable evidence, add any missing complete migration fixture/fixed-EN boundary tests, and close #238 only when every acceptance item is proven.
