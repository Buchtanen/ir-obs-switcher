...
specification starts from current `master`. This document does not reimplement
N12. Overlapping iRSDK reads and latest-state publication remain the next
target.
...
- N12 already isolates accepted-event delivery; it does not define how
  consumers obtain changing live state economically;
...
- This specification does not reimplement N12 or change current runtime behavior.
...
independent overlay/commentary workers. This specification extends that shipped
N12 design with:
...
| --- | --- | --- |
| M0 | Add baseline read/queue/state/WS metrics on current N12 `master` | recorded baseline |
...
- [ ] Implementation starts from current N12-on-`master`. This spec is the
      post-N12 sampling target, not an N12 replacement.
...
- weather/static/system can be slower, fast race detection cannot be casually
  decimated;
- N12 is already on `master` and remains authoritative for async event delivery.
