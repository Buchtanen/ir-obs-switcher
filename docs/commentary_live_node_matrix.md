...
| `hunted` | `HUNTED` | `gap`, `target_name`, `position` | **yes** — BattleEmitter → battle adapter | same battle metrics | **low–medium** — live DATA is `{chaser, target, gap, situation}` (no `relation` leak; reject `Target's`) |
| `side_by_side` | `SIDE_BY_SIDE`, `BATTLE_FOR_POSITION` | `position`, `target_name` | **yes** — BattleEmitter | `gap`, `position`, `targetCarIdx`, `targetName`, … | **low–medium** — `two_front_battle` live DATA `{hero, car_ahead, car_behind, gap_ahead, gap_behind}` (2-decimal `s`); director fills missing `hero_name` |
| `overtake` | `OVERTAKE` | `position`, `target_name` | **partial** — classifier when flagged; adapter keeps `targetName` / target subject | `oldPosition`, `newPosition`, `delta`, `targetName`, … | **low** when name present; director fills missing `hero_name` so typed `{hero, passed}` DATA still fires |
| `position_gained` | `POSITION_GAINED` | `position`, `old_position`, `places` | **yes** — swing tracker (gain and loss coalesce; longer debounce after incident / multi-place) → `adapters/position.py` | `direction`, `oldPosition`, `newPosition`, `delta`, `places` | **low** — slots map from metrics; live DATA uses `from`/`to`/`places` when `places>=2` |
| `position_lost` | `POSITION_LOST`, `OVERTAKEN` | `position`, `old_position`, `places` | **yes** — same swing tracker as gain; **`OVERTAKEN` never emitted** | `direction`, `oldPosition`, `newPosition`, `delta`, `places` | **low** for `POSITION_LOST`; `OVERTAKEN` path **n/a** |
...
| `leader_change` | `LEADER_CHANGE` | `target_name`, `leader_name`, `position` | **yes** — `LeaderChangeEmitter` (`events/leader_change.py`, prio 75) → position adapter; overlay plate reuses `position_gained` | `oldLeaderName`, `targetName`, `targetCarIdx`, `position`, `p1Name` | **low** — composer has name-only fallback; HUD copy `position.leader_change` |
...
| `incident` | `INCIDENT` | `value` | **yes** — `IncidentEmitter` → `adapters/exception_extra.py` | `value`, `total` | **low** — live DATA `{hero, incident_points, situation: "incident points"}`; hold slot survives a deferred hunting flush |
...
| `finish` | `FINISH` | `position` | **yes** — `SessionEmitter` → session adapter | `position`, `classPosition` | **low** — live DATA `{hero?, situation: "race is complete", position?}` |
...
| `stream_start` | `STREAM_START` | *(none)* | **yes** — commentary-only; OBS rising edge **or already-live at connect** → `notify_obs_stream_started` (`commentary.stream_start`) | none | **none** — slot-free; node `tts.max_seconds` ≥ 15 |
| `stream_end` | `STREAM_END` | *(none)* | **yes** — commentary-only; iRacing QUIT → `notify_sim_quit` then existing auto-stop | none | **none** — slot-free thanks / next-broadcast wish |
