...
5) **Handover checkpoint**
   - Až teď `/handover`: trvalý stav (worktree, HEAD, TDD fáze, next), aby další agent neztratil kontext. Na v2 větvi i `docs/v2.0.0/implementation-handover.md`.

6) **PR do master**
   - Pokud issue-set nebo lidský prompt nestanoví jinak: otevři/aktualizuj PR **do `master`**.
   - v2 (`codex/commentary-story-flow-spec`): **žádný PR do `master`** do cutoveru.
   - Popis podle template (AC/test plan/docs impact/config impact).
   - Přesně jeden `semver:*` label (jen když PR do `master` existuje).
